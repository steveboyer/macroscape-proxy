...
Sign in with Apple callers (id_token in `Authorization: Bearer`), auto-creates a user record on
...
- **`MacroScapeProxyStack`** (`lib/macroscape-proxy-stack.ts`) — the runtime proxy. Wires:
  ...
  Handler env vars: `TABLE_NAME`, `UPSTREAM_SECRET_ARN`, `APPLE_SIGNIN_SECRET_ARN`, `APPLE_AUD`
  (currently `app.macroscape.MacroScape` — the iOS Bundle ID; JWTs whose `aud` doesn't match are
  rejected), `DEFAULT_DAILY_LIMIT` (currently `100`).
